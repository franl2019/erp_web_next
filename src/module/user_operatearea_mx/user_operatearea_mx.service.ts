...
export class User_operatearea_mxService {
    ...
    public async findDefaultUserOperateArea(){
        const result = await http_post<IApiResult<IUser_operatearea_mx>>(API_URL.USER_OPERATEAREA_MX_DEFAULT);
    ...
    public async create(createDto: ICreateUserOperateAreaDto) {
        const result = await http_post<IApiResult>(API_URL.USER_OPERATEAREA_MX_ADD, createDto);
        if (result.code === 200) {
            return true
        } else {
    ...
    public async delete_data(deleteDto: IDeleteUserOperateAreaDto) {
        const result = await http_post<IApiResult>(API_URL.USER_OPERATEAREA_MX_DELETE, deleteDto);
        if (result.code === 200) {
            return true
        } else {
