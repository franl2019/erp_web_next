<template>
  <erp-page-box>
    <erp-no-title>
      <erp-button :disabled="!state.edit" type="info" v-reqClick="clickedSaveBtn">保存</erp-button>
    ...
    <erp-form>
      <erp-form-item v-if="editDto.accountInComeCode" label-for-name="单据编号" md-col="2" lg-col="1">
      ...
      <erp-form-item label-for-name="客户" md-col="2" lg-col="2">
        <erp-select-client-btn
            :clientname="editDto.clientname"
            @select="onSelectClient"
            @unSelect="onUnSelectClient"
            :disabled="!state.edit">
        </erp-select-client-btn>
      </erp-form-item>
      <erp-form-item label-for-name="总金额(自动计算)" md-col="2" lg-col="1">
        <erp-input-round v-model="editDto.amount" disabled></erp-input-round>
      </erp-form-item>
      <erp-form-item label-for-name="发生日期" md-col="2" lg-col="1">
        <el-date-picker v-model="editDto.indate" type="date" value-format="YYYY-MM-DD HH:mm:ss"
                        placeholder="选择日期" :disabled="!state.edit"></el-date-picker>
      </erp-form-item>
    ...
    <erp-title title="收款明细">
      <template #default>
        ...
        <erp-button v-show="state.edit" @click="clickedDeleteSelectAmountMx" type="danger">删除明细</erp-button>
      </template>
    </erp-title>

    <erp-table
        ref="editAccountInComeAmountMxTableRef"
        :table-state="editAccountInComeAmountMxTableConfig"
        :find-dto="{}"
        :table-edit="state.edit"
        :getRowNodeId="getRowNodeIdForAmountMx"
        @selection-changed=""
        @cellValueChanged="cellValueChangedAmountMx"
    >
    </erp-table>
    ...
    <erp-title title="核销明细">
      <template #default>
        ...
        <erp-button v-show="state.edit" @click="clickedDeleteSheetMx" type="danger">删除明细</erp-button>
      </template>
    </erp-title>

    <erp-table
        ref="editAccountInComeSheetMxTableRef"
        :table-state="editAccountInComeSheetMxTableConfig"
        :find-dto="{}"
        :table-edit="state.edit"
        :getRowNodeId="getRowNodeIdForSheetMx"
        @selection-changed=""
        @cellValueChanged=""
    >
    </erp-table>
