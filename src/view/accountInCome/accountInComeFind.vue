    ...
    <erp-no-title title="销售收款单">
      ...
      <template v-slot:input>
        <el-date-picker v-model="findDate" end-placeholder="结束日期" range-separator="-" start-placeholder="开始日期"
          type="daterange" unlink-panels value-format="YYYY-MM-DD HH:mm:ss" @change="initPage"></el-date-picker>
      </template>
    </erp-no-title>
    <erp-table ref="accountInComeFindTableRef" :getRowNodeId="getHeadTableRowNodeId" :find-dto="accountInComeFindDto"
      :table-state="defaultAccountInComeTableConfig" @selectionChanged="selectionChanged">
    </erp-table>

    <erp-title title="收款明细">
    </erp-title>

    <erp-table ref="accountInComeAmountMxFindTableRef" :find-dto="accountInComeAmountMxFindDto"
