<template>
  <erp-page-box>
    <erp-title title="辅助单位管理">
      <erp-button
          :disabled="!buttonShowState.create"
          type="info"
          @click="onShowCreateProductOtherUnitDialog"
      >
        新增
      </erp-button>
      <erp-button
          :disabled="!buttonShowState.edit"
          type="success"
          @click="onShowUpdateProductOtherUnitDialog"
      >
        修改
      </erp-button>
      <erp-button
          :disabled="!buttonShowState.delete_data"
          type="danger"
...
function selectionChanged(event: SelectionChangedEvent) {
  ...
  updateButtonState(productOtherUnitSelect.value?.level1Review, productOtherUnitSelect.value?.level2Review);
}
...
async function onClickDeleteButton(){
  ...
  ErpDialog({
    ...
    ok:async ()=>{
      ...
      deleteDto.productOtherUnitId = productOtherUnitSelect.value?.productOtherUnitId
