...
import createWarehousePermissionsDialog
  from "@/view/auth/userWarehouseMx/component/createWarehousePermissionsDialog.vue";
import {
  defaultUserWarehouseMxTableState
} from "@/view/auth/userWarehouseMx/tableState/defaultUserWarehouseMxTableState";
    ...
    async function onClickedDeleteBtn() {
      if (Number(props.userid) !== 0) {
        ...
        if (rowClickedValue.value) {
          ...
          await userWarehousePermissionTableRef.value?.initTableData();
          rowClickedValue.value = undefined
        }
      }
    }
