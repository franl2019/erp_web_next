<template>
  <erp-page-box>
    <erp-no-title>
      <erp-button :disabled="!state.edit" type="info" v-reqClick="clickedSaveButton">保存</erp-button>
    ...
    <erp-form>
      ...
      <erp-form-item label-for-name="核销类型" lg-col="1" md-col="2">
        <erp-accounts-verify-sheet-type-select v-model="editDto.sheetType"
                                               ...
                                               :disabled="!state.edit"></erp-accounts-verify-sheet-type-select>
      </erp-form-item>
      <erp-form-item label-for-name="发生日期" lg-col="1" md-col="2">
        <el-date-picker v-model="editDto.inDate" placeholder="选择日期" type="date"
                        value-format="YYYY-MM-DD HH:mm:ss" :disabled="!state.edit"></el-date-picker>
      </erp-form-item>
      <erp-form-item v-if="inputVisible.client" label-for-name="客户" lg-col="2" md-col="2">
        <erp-select-client-btn :clientname="editDto.clientname" @select="onSelectClient"
                               @unSelect="onUnSelectClient" :disabled="!state.edit"></erp-select-client-btn>
      </erp-form-item>
      <erp-form-item v-if="inputVisible.client_b" label-for-name="转入客户" lg-col="2" md-col="2">
        <erp-select-client-btn :clientname="editDto.clientname_b" @select="onSelectClient_b"
                               @unSelect="onUnSelectClient_b" :disabled="!state.edit"></erp-select-client-btn>
      </erp-form-item>
      <erp-form-item v-if="inputVisible.buy" label-for-name="供应商" lg-col="2" md-col="2">
        <erp-select-buy-btn :buyname="editDto.buyname" @select="onSelectBuy"
                            @unSelect="onUnSelectBuy" :disabled="!state.edit"></erp-select-buy-btn>
      </erp-form-item>
      <erp-form-item v-if="inputVisible.buy_b" label-for-name="转入供应商" lg-col="2" md-col="2">
        <erp-select-buy-btn :buyname="editDto.buyname_b" @select="onSelectBuy_b"
                            @unSelect="onUnSelectBuy_b" :disabled="!state.edit"></erp-select-buy-btn>
      </erp-form-item>
    ...
    <erp-title title="核销明细">
      ...
      <erp-button v-show="state.edit && editDto.sheetType === 3" @click="clickedAddAccountsReceivableToSheetMxBtn">选应收</erp-button>
      <erp-button v-show="state.edit && editDto.sheetType === 3" @click="clickedAddAccountsPayableToSheetMxBtn">选应付</erp-button>
      <erp-button v-show="state.edit" type="danger" @click="deleteSheetMxToSelected">删除明细</erp-button>
    </erp-title>

    <erp-table
        ref="accountsVerifySheetMxEditRef"
        :find-dto="{}"
        :getRowNodeId="getRowNodeId"
        :table-edit="state.edit"
        :table-state="editAccountsVerifySheetMxTableConfig"
        @selection-changed="()=>{}"
    >
    </erp-table>
  </erp-page-box>
...
import ErpSelectAccountReceivableDialog
  from "@/components/dialog/selectInfo/account/accountReceivable/selectAccountReceivableDialog";
